...
: Node("laser_segmentation"), setup_(false)
{
  ...
  RCLCPP_INFO(
    this->get_logger(), "The parameter min_avg_distance_from_sensor is set to: [%f]",
  ...
  RCLCPP_INFO(
    this->get_logger(), "The parameter max_avg_distance_from_sensor is set to: [%f]",
  ...
  RCLCPP_INFO(
    this->get_logger(), "The parameter min_segment_width is set to: [%f]", min_segment_width_);
  ...
  RCLCPP_INFO(
    this->get_logger(), "The parameter max_segment_width is set to: [%f]", max_segment_width_);
  ...
  RCLCPP_INFO(
    this->get_logger(), "The parameter distance_threshold is set to: [%f]", distance_thres_);
  ...
  RCLCPP_INFO(
    this->get_logger(), "The parameter noise_reduction is set to: [%f]",
    noise_reduction_);
  ...
  RCLCPP_INFO(
    this->get_logger(), "The parameter restore_defaults is set to: [%s]",
    restore_ ? "true" : "false");
  ...
  RCLCPP_INFO(
    this->get_logger(), "The parameter method_threshold is set to: [%s]", method_thres_.c_str());
  ...
  RCLCPP_INFO(
    this->get_logger(), "The parameter segment_topic is set to: [%s]",
    seg_topic_.c_str());
  ...
  RCLCPP_INFO(
    this->get_logger(), "The parameter segmentation_type is set to: [%s]",
  ...
  const std::vector<rclcpp::Parameter> & parameters)
{
  ...
  for (const auto & param : parameters) {
      ...
      param.get_type() == rclcpp::ParameterType::PARAMETER_INTEGER)
    {
      ...
      RCLCPP_INFO(
        this->get_logger(), "The parameter min_points_segment is set to: [%d]", min_points_);
    }
      ...
      param.get_type() == rclcpp::ParameterType::PARAMETER_INTEGER)
    {
      ...
      RCLCPP_INFO(
        this->get_logger(), "The parameter max_points_segment is set to: [%d]", max_points_);
    }
    // FLOAT PARAMS ..........................................................................
      ...
      param.get_type() == rclcpp::ParameterType::PARAMETER_DOUBLE)
    {
      ...
      RCLCPP_INFO(
        this->get_logger(), "The parameter min_avg_distance_from_sensor is set to: [%3.3f]",
        min_avg_distance_from_sensor_);
    }
      ...
      param.get_type() == rclcpp::ParameterType::PARAMETER_DOUBLE)
    {
      ...
      RCLCPP_INFO(
        this->get_logger(), "The parameter max_avg_distance_from_sensor is set to: [%3.3f]",
        max_avg_distance_from_sensor_);
    }
      ...
      param.get_type() == rclcpp::ParameterType::PARAMETER_DOUBLE)
    {
      ...
      RCLCPP_INFO(
        this->get_logger(), "The parameter min_segment_width is set to: [%3.3f]",
        min_segment_width_);
    }
      ...
      param.get_type() == rclcpp::ParameterType::PARAMETER_DOUBLE)
    {
      ...
      RCLCPP_INFO(
        this->get_logger(), "The parameter max_segment_width is set to: [%3.3f]",
        max_segment_width_);
    }
      ...
      param.get_type() == rclcpp::ParameterType::PARAMETER_DOUBLE)
    {
      ...
      RCLCPP_INFO(
        this->get_logger(), "The parameter distance_threshold is set to: [%3.3f]", distance_thres_);
    }
      ...
      param.get_type() == rclcpp::ParameterType::PARAMETER_DOUBLE)
    {
      distance_thres_ = param.as_double();
      RCLCPP_INFO(
        this->get_logger(), "The parameter noise_reduction is set to: [%3.3f]", noise_reduction_);
    }
      ...
      param.get_type() == rclcpp::ParameterType::PARAMETER_STRING)
    {
      ...
      RCLCPP_INFO(
        this->get_logger(), "The parameter method_threshold is set to: [%s]",
        method_thres_.c_str());
    }
  }
...
void laserSegmentation::scan_callback(const sensor_msgs::msg::LaserScan::SharedPtr scan_msg)
{
  ...
  segmentation_->initialize_segmentation(
    distance_thres_, angle_resolution, noise_reduction_,
    method_thres_);
  ...
  for (const auto & segment : segment_list) {
    ...
    if (segment.centroid().length() < min_avg_distance_from_sensor_ ||
      segment.centroid().length() > max_avg_distance_from_sensor_) {continue;}

    // By width
    if (segment.width_squared() < squared_min_segment_width ||
      segment.width_squared() > squared_max_segment_width) {continue;}
